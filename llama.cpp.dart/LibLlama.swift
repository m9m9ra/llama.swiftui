...
actor LlamaContext {
    ...
    init(
        ...
        self.tokens_list = []
        self.batch = llama_batch_init(Int32(n_batch), 0, 1)
...
//         llama_backend_free()
    ...
    func inference_init(message_list: [LlamaChatMessage], add_bos: Bool = true, clear_token: Bool = true) {
        ...
        // Добавляем токены с правильными позициями
        ...
        if llama_decode(context, batch) != 0 {
            print("llama_decode() failed init")
        }
    ...
    func inference_loop() -> String {
        ...
        // Используйте последний токен в батче
        let last_token_index = batch.n_tokens - 1
        guard last_token_index >= 0 else {
        ...
        // Подготовка батча для следующего декодирования
        llama_batch_clear(&batch)
    ...
    public func tokenize(text: String, add_bos: Bool, clear_token: Bool = true) -> [llama_token] {
...
//        let tokenCount = llama_tokenize(vocab, text, Int32(utf8Count), tokens, Int32(n_tokens), add_bos, clear_token)
        let tokenCount = llama_tokenize(vocab, strdup(text), Int32(utf8Count), tokens, Int32(n_tokens), add_bos, clear_token)
