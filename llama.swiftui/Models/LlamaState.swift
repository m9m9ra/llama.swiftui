...
class LlamaState: ObservableObject {
    ...
    func loadModelFromFile(modelUrl: String?) throws {
        if let modelUrl {
            messageLog += "Loading model...\n"
            llamaContext = try LlamaContext.create_context(model_path: modelUrl)
            messageLog += "Loaded model \(modelUrl)\n"
        } else {
    ...
    func loadModel(modelUrl: URL?) throws {
        if let modelUrl {
            messageLog += "Loading model...\n"
            llamaContext = try LlamaContext.create_context(model_path: modelUrl.path())
